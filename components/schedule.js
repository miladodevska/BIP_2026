...
export function createSchedule() {
    ...
    const onlineDays = CONFIG.ONLINE;
    return `
            ...
            <div class="grid lg:grid-cols-3 gap-12">
             <div class="lg:col-span-1">
                 ...
                 <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-[10px] font-bold tracking-widest uppercase mb-6 border border-slate-200">
                     Phase 01: Online Component</div>
                     <div class="border border-slate-100 rounded-[2rem] overflow-hidden bg-slate-50 hover:border-blue-200 transition-all">                         
                        <button data-day-toggle="phase01" type="button" class="w-full text-left p-8 flex items-center justify-between group">                             
                        <div>                                 
                            <span class="text-xs font-bold text-blue-600 uppercase tracking-widest">Phase 01</span>                                 
                            <h5 class="text-xl font-bold text-slate-900">Online Lectures</h5>
                        </div>
                        <div id="icon-phase01" class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all">
                         ...
                         <div id="content-phase01" class="hidden px-8 pb-8 animate-fadeIn">
                             <div class="pt-4 border-t border-slate-200/60 space-y-6">
                                 ${CONFIG.ONLINE.map((day, index) => `
                                     <div class="space-y-3">
                                         <span class="text-xs font-bold text-blue-600 uppercase tracking-widest">Day ${index + 1} — ${day.title}</span>
                                         ${day.sessions.map(session => `
                                             <div class="flex gap-4">
                                                 <span class="mono text-blue-600 font-bold text-sm">${session.time}</span>
